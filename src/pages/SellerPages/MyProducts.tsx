...
const MyProducts = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!editing) body.sellerId = user.id;

      const url = editing
        ? `http://localhost:8080/api/seller/products/${editing.id}`
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/seller/products/${id}`, { method: 'DELETE' });
      ...
      {products.length === 0 ? (
        <div className="admin-card">
          <div className="admin-empty">
            ...
            <p className="admin-empty-title">Bạn chưa có sản phẩm nào</p>
            <p className="admin-empty-desc">Hãy thêm sản phẩm đầu tiên để bắt đầu bán hàng</p>
            <button onClick={openCreate} className="admin-btn-primary">+ Thêm sản phẩm</button>
          </div>
        </div>
      ) : (
        <div className="admin-card">
              ...
              <tbody>
                {products.map(p => (
                  <tr key={p.id}>
                    <td>
                      <div className="flex items-center gap-3">
                        <img src={p.img || '/assets/netflix-logo.png'} alt={p.name}
                    ...
                    <td className="text-right">
                      <p className="text-sm font-semibold text-gray-900">{p.price?.toLocaleString('vi-VN')}đ</p>
                      {p.priceOri && <p className="text-xs text-gray-400 line-through">{p.priceOri.toLocaleString('vi-VN')}đ</p>}
                    </td>
                    <td className="text-center text-sm font-medium">{p.sold || 0}</td>
                    <td className="text-center">
                      <span className="admin-tag">★ {p.rating || 0}</span>
                    </td>
                    <td className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(p)} className="admin-btn-icon text-gray-400 hover:text-blue-600 hover:bg-blue-50">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                          </svg>
                        </button>
                        <button onClick={() => handleDelete(p.id)} className="admin-btn-icon text-gray-400 hover:text-red-600 hover:bg-red-50">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="admin-modal max-w-lg">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold text-gray-900">{editing ? 'Sửa sản phẩm' : 'Thêm sản phẩm'}</h3>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input type="text" value={form.img} onChange={e => setForm({ ...form, img: e.target.value })}
                      className="admin-input flex-1" placeholder="URL ảnh" />
                  ...
                  {form.img && <img src={form.img} alt="preview" className="mt-3 w-16 h-16 rounded-xl object-cover border border-gray-200 shadow-sm" />}
                </div>
                {categories.length > 0 && (
                  <div>
